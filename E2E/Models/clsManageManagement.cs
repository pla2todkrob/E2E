...
namespace E2E.Models
{
    public class ClsManageManagement
    {
        ...
        protected async Task<bool> Document_Insert(ClsDocuments model, HttpFileCollectionBase files)
        {
            try
            {
                ...
                if (files[0].ContentLength != 0)
                {
                    ...
                    master_DocumentVersions.DocumentVersion_Path = await clsManageService.UploadFileToString(dir, file, FileName);
        ...
        protected async Task<bool> Document_Update(ClsDocuments model, HttpFileCollectionBase files)
        {
            ...
            {
                bool res = new bool();
                Master_Documents master_Documents = await db.Master_Documents
                ...
                if (files[0].ContentLength != 0)
                {
                    ...
                    Master_DocumentVersions master_DocumentVersions = new Master_DocumentVersions
                    {
                        Document_Id = master_Documents.Document_Id,
                        DocumentVersion_Name = FileName,
                        DocumentVersion_Path = await clsManageService.UploadFileToString(dir, file, FileName),
                        User_Id = userId,
                        DocumentVersion_Number = Count
                    };
                    db.Master_DocumentVersions.Add(master_DocumentVersions);
                }
        ...
        public async Task<bool> Document_Save(ClsDocuments model, HttpFileCollectionBase files)
        {
            try
            {
                ...
                Master_Documents master_Documents = await db.Master_Documents.Where(w => w.Document_Id == model.Master_Documents.Document_Id).FirstOrDefaultAsync();
        ...
        public bool WorkRoot_Delete(Guid id)
        {
            try
            {
                ...
                if (inUseCount == 0)
                {
                    ...
                    db.Entry(workRoots).State = EntityState.Deleted;
                    if (db.SaveChanges() > 0)
                    {
                        res = true;
                    }
        ...
        public bool WorkRoot_Insert(ClsWorkRoots model)
        {
            try
            {
                ...
                db.Entry(workRoots).State = EntityState.Added;
        ...
        public bool WorkRoot_Update(ClsWorkRoots model)
        {
            try
            {
                ...
                db.Entry(workRoots).State = EntityState.Modified;
                    ...
                    if (model.Document_Id.Count > 0)
                    {
                        ...
                        foreach (var item in workRootDocuments)
                        {
                            ...
                            if (findKeep == null)
                            {
                                ...
                                db.Entry(documents).State = EntityState.Deleted;
                            }
                        }
                    }
                    else
